...
emperor_killers %>% 
  mutate(
    is_other_emp = ifelse(killer == "Other Emperor", TRUE, FALSE),
    # use `fct_inorder()` to maintain the way we sorted the data
    killer = fct_inorder(killer)
  ) %>%
  ggplot(aes(x = n, y = killer, fill = is_other_emp)) +
  geom_col() +
...
coffee_ratings %>% 
  mutate(mean_rating = mean(aroma+flavor+aftertaste+acidity+acidity+body+balance+uniformity+)
